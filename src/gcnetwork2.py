...
def _resNetBlock_(input, filters, ksize, stride, padding, act_func):
    conv1 = Conv2D(filters, ksize, strides = stride, padding = padding)(input)
    bn1 = BatchNormalization()(conv1)
    act1 = Activation(act_func)(bn1)
    conv2 = Conv2D(filters,ksize, strides = stride, padding = padding)(act1)
    bn2 = BatchNormalization()(conv2)
    act2 = Activation(act_func)(bn2)
    return add([input, act2])
...
def _createUniFeature_(input, num_res, filters, first_ksize, ksize, act_func, ds_stride, padding):
    ...
    conv1 = Conv2D(filters, first_ksize, strides = ds_stride, padding = padding, input_shape = input_shape)(input)
    bn1 = BatchNormalization()(conv1)
    res_block = Activation(act_func)(bn1)
    for i in range(num_res):
        res_block = _resNetBlock_(res_block, filters, ksize, 1, padding, act_func)
    unifeature = Conv2D(filters, ksize, strides = 1, padding = padding)(res_block)
    return unifeature
...
def createGCNetwork(hp, tp, pre_weight):
    ...
    left_feature = _createUniFeature_(left_img, num_res, num_filters, first_ksize, ksize, act_func, ds_stride, padding)
    left_feature_model = Model(left_img, left_feature)
    right_feature = left_feature_model(right_img)
    unifeatures = [left_feature, right_feature]
