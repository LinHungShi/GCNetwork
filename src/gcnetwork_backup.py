...
def _getCostVolume_(inputs, d):
	...
	shape = K.shape(lf)
        b, f, h, w = K.int_shape(lf)
        rf = states[0]
        rfs = rf[:, :, :, :-1]
        disp_rfs = K.spatial_2d_padding(rfs, padding = ((0, 0), (1, 0)))
        output = K.concatenate([lf, rf], axis = 2)
        output = K.reshape(output, (shape[0], 2 * f, shape[2], shape[3]))
    ...
    l,o,n = K.rnn(featureConcat, inputs = left_feature, initial_states = [right_feature], unroll = False)
    return K.permute_dimensions(o, (0,2,1,3,4));

def  _computeLinearScore_(cv, d):
    #cv = K.squeeze(cv, axis = 1)
    disp_mat = K.expand_dims(K.arange(d, dtype = K.floatx()), axis = 0)
...
def _computeSoftArgMin_(cv, d):
    ...
    #disp_mat = K.expand_dims(K.arange(d, dtype = 'float32'), axis = 1)
    #softmax = K.permute_dimensions(softmax, (0,1,3,2))
    #output = K.dot(softmax, disp_mat)
    softmax = tf.nn.softmax(-cv, dim = 1)
    disp_mat = K.reshape(K.arange(0, d, dtype = K.floatx()), (1,1,d,1))
    output = K.conv2d(softmax, disp_mat, strides = (1,1), padding = 'valid')
...
def _LearnReg_(input, base_num_filters, ksize, ds_stride, resnet, padding, highway_func, num_down_conv):    
    ...
    output = _createDeconv3D_(up_convs, 1, ksize, ds_stride, padding, bn = False, act_func = None)
    output = Lambda(K.squeeze, arguments = {'axis': 1})(output)
    return output

def createGCNetwork(hp):
    d = hp['max_disp']
    resnet = hp['resnet']
    ...
    left_img = Input((3, None, None), dtype = K.floatx())
    right_img = Input((3, None, None), dtype = K.floatx())
    ...
    cv = Lambda(_getCostVolume_, arguments = {'d':d/2})(unifeature)
